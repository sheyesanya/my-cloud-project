...
const scaleUp  = { hidden:{opacity:0,scale:0.95}, show:{opacity:1,scale:1,transition:{duration:0.45,ease:[0.22,1,0.36,1]}} };
...
function FadeUp({ children, delay=0, style={}, className='' }) {
  ...
  return (
    <div ref={ref} className={className}
      style={{ opacity:visible?1:0, transform:visible?'translateY(0)':'translateY(20px)', transition:`opacity 0.55s cubic-bezier(0.22,1,0.36,1) ${delay}s, transform 0.55s cubic-bezier(0.22,1,0.36,1) ${delay}s`, ...style }}>
      {children}
    </div>
  );
}
...
export default function Login() {
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#080810', color:'white', fontFamily:'Manrope,sans-serif', overflowX:'hidden', width:'100%' }}>
      ...
      {/* NAV */}
      <motion.nav initial={{ opacity:0, y:-10 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.4, ease:[0.22,1,0.36,1] }}
        style={{ position:'sticky', top:0, zIndex:200, background:'rgba(8,8,16,0.95)', backdropFilter:'blur(16px)', borderBottom:'1px solid rgba(255,255,255,0.07)' }}>
        <div className="px" style={{ height:60, display:'flex', alignItems:'center', justifyContent:'space-between', maxWidth:1160, margin:'0 auto' }}>
          ...
          <div className="nav-desktop" style={{ alignItems:'center', gap:28 }}>
            {[['Features','features'],['How it works','how-it-works'],['For Providers','providers']].map(([l,id]) => (
              <button key={l} onClick={() => scrollTo(id)} style={{ background:'none', border:'none', color:'rgba(255,255,255,0.55)', fontSize:13, fontWeight:600, cursor:'pointer', fontFamily:'Manrope,sans-serif' }}>{l}</button>
            ))}
          </div>
        ...
        {menuOpen && (
          <div style={{ background:'rgba(8,8,16,0.98)', borderTop:'1px solid rgba(255,255,255,0.07)', padding:'8px 0 16px' }}>
            {[['Features','features'],['How it works','how-it-works'],['For Providers','providers']].map(([l,id]) => (
              <button key={l} onClick={() => scrollTo(id)} style={{ display:'block', width:'100%', textAlign:'left', padding:'13px 20px', background:'none', border:'none', borderBottom:'1px solid rgba(255,255,255,0.05)', color:'rgba(255,255,255,0.75)', fontSize:15, fontWeight:600, cursor:'pointer', fontFamily:'Manrope,sans-serif' }}>{l}</button>
            ))}
            <Link to="/register/provider" onClick={() => setMenuOpen(false)} style={{ display:'block', padding:'13px 20px', color:'#5eead4', fontSize:15, fontWeight:700, textDecoration:'none' }}>
              List your media →
            </Link>
          </div>
        )}
        ...
        <div className="hero-wrap">
          ...
          <motion.div variants={stagger} initial="hidden" animate="show">
            <motion.div variants={fadeUp}
              style={{ display:'inline-flex', alignItems:'center', gap:8, padding:'5px 13px', borderRadius:20, background:'rgba(99,102,241,0.1)', border:'1px solid rgba(99,102,241,0.22)', marginBottom:22 }}>
            ...
            <motion.h1 variants={fadeUp} className="hero-h1" style={{ fontWeight:800, lineHeight:1.08, marginBottom:18 }}>
              Book media campaigns<br/>
              <span className="grad">across Nigeria</span>
            </motion.h1>

            <motion.p variants={fadeUp} style={{ fontSize:16, color:'rgba(255,255,255,0.5)', lineHeight:1.75, marginBottom:28 }}>
              One platform to discover, book and manage campaigns across TV, radio, podcasts, billboards, print and influencers.
            </motion.p>

            <motion.div variants={fadeUp}
              className="stats-wrap" style={{ marginBottom:28, padding:'18px 22px', borderRadius:14, background:'rgba(255,255,255,0.02)', border:'1px solid rgba(255,255,255,0.06)' }}>
              {STATS.map(s => (
                <div key={s.label}>
            ...
            <motion.div variants={fadeUp} style={{ display:'flex', gap:10, flexWrap:'wrap' }}>
              <button onClick={() => { setMode('signup'); scrollTo('auth-panel'); }} style={{ padding:'12px 26px', borderRadius:12, background:'linear-gradient(135deg,#6366f1,#a855f7)', color:'white', fontWeight:700, fontSize:15, border:'none', cursor:'pointer', fontFamily:'Manrope,sans-serif' }}>
                Start for free →
              </button>
              <Link to="/register/provider" style={{ padding:'12px 20px', borderRadius:12, background:'rgba(255,255,255,0.04)', color:'rgba(255,255,255,0.65)', fontWeight:600, fontSize:14, border:'1px solid rgba(255,255,255,0.1)', textDecoration:'none', display:'inline-flex', alignItems:'center' }}>
                List your inventory
              </Link>
            </motion.div>
          </motion.div>
          ...
          <motion.div variants={fadeRight} initial="hidden" animate="show" id="auth-panel"
            style={{ background:'#0d0d1a', border:'1px solid rgba(255,255,255,0.09)', borderRadius:20, padding:26, position:'relative', overflow:'hidden', width:'100%' }}>
            ...
            <div style={{ textAlign:'center', marginBottom:18 }}>
              ...
              <p style={{ fontWeight:800, fontSize:18, letterSpacing:'-0.3px', marginBottom:3 }}>
                {isProvider ? 'Provider Sign In' : mode === 'login' ? 'Welcome back' : 'Get started free'}
              </p>
              <p style={{ fontSize:12, color:'rgba(255,255,255,0.35)' }}>
                {isProvider ? 'Access your media provider dashboard' : mode === 'login' ? 'Sign in to your brand dashboard' : 'Create your BrandCasta account'}
              </p>
            </div>
            ...
            <div style={{ display:'flex', gap:3, background:'rgba(255,255,255,0.04)', borderRadius:10, padding:3, marginBottom:16, border:'1px solid rgba(255,255,255,0.07)' }}>
              {[['login','Sign In'],['signup','Sign Up'],['provider','Provider']].map(([m, label]) => (
                <button key={m} onClick={() => setMode(m)}
                  style={{ flex:1, padding:'8px 4px', borderRadius:7, fontSize:12, fontWeight:700, cursor:'pointer', fontFamily:'Manrope,sans-serif', transition:'all 0.15s', border:'none',
                    background: mode === m ? m === 'provider' ? 'linear-gradient(135deg,#14b8a6,#06b6d4)' : 'linear-gradient(135deg,#6366f1,#a855f7)' : 'transparent',
            ...
            {error && (
              <div style={{ padding:'10px 12px', borderRadius:8, background:'rgba(239,68,68,0.08)', border:'1px solid rgba(239,68,68,0.18)', color:'#fca5a5', fontSize:12, marginBottom:12 }}>
                {error}
              </div>
            )}
            ...
            <button onClick={() => {
                if (mode === 'signup' && !agreedToTerms) { setError('Please agree to the Terms & Conditions before continuing with Google.'); return; }
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
              ...
              <button
                onClick={() => {
                  if ((mode === 'signup') && !agreedToTerms) { setError('Please agree to the Terms & Conditions to create an account.'); return; }
                  handle(() => mode === 'login' || isProvider ? login(email, password) : signup(email, password));
                }}
                disabled={loading}
                ...
                {loading
                  ? <><Spinner size={13}/>{isProvider || mode === 'login' ? 'Signing in…' : 'Creating account…'}</>
                  : isProvider ? 'Service Provider Sign In →' : mode === 'login' ? 'Sign In →' : 'Create Free Account →'}
              </button>
            </div>
            ...
            <div style={{ borderTop:'1px solid rgba(255,255,255,0.06)', marginTop:16, paddingTop:14, textAlign:'center' }}>
              {!isProvider ? (
                <>
                  ...
                  <Link to="/register/provider" style={{ display:'inline-flex', alignItems:'center', gap:6, padding:'8px 16px', borderRadius:9, background:'rgba(20,184,166,0.07)', border:'1px solid rgba(20,184,166,0.2)', color:'#5eead4', fontWeight:700, fontSize:12, textDecoration:'none' }}>
                    Apply as a Service Provider →
                  </Link>
                </>
              ) : (
                <>
                  <p style={{ fontSize:11, color:'rgba(255,255,255,0.25)', marginBottom:10 }}>Not yet a provider?</p>
                  <Link to="/register/provider" style={{ display:'inline-flex', alignItems:'center', gap:6, padding:'8px 16px', borderRadius:9, background:'rgba(20,184,166,0.07)', border:'1px solid rgba(20,184,166,0.2)', color:'#5eead4', fontWeight:700, fontSize:12, textDecoration:'none' }}>
          ...
          <div className="feat-wrap">
            {FEATURES.map((f, i) => (
              <FadeUp key={f.title} delay={i * 0.07}>
                <div className="feat-card" style={{ padding:'20px', borderRadius:14, background:'#0d0d1a', border:'1px solid rgba(255,255,255,0.07)', height:'100%' }}>
            ...
            <AnimatePresence mode="wait">
              {TESTIMONIALS.map((t, i) => i === activeTestimonial && (
                <motion.div key={i}
                  ...
                  style={{ padding:'24px', borderRadius:16, background:'#0d0d1a', border:'1px solid rgba(255,255,255,0.08)', textAlign:'left' }}>
            ...
            <div>
              ...
              <div style={{ display:'flex', gap:8 }}>
                {[['𝕏','https://x.com/brandcasta_ng'],['in','https://linkedin.com/company/brandcasta-nigeria'],['ig','https://instagram.com/brandcasta_ng']].map(([icon,href]) => (
                  <a key={icon} href={href} target="_blank" rel="noreferrer" style={{ width:30, height:30, borderRadius:8, background:'rgba(255,255,255,0.05)', border:'1px solid rgba(255,255,255,0.1)', display:'flex', alignItems:'center', justifyContent:'center', color:'rgba(255,255,255,0.5)', fontSize:11, fontWeight:700, textDecoration:'none' }}>
                    {icon}
                  </a>
                ))}
              </div>
